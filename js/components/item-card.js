...
class ItemCard {
    ...
    static render(name, data, category) {
        ...
        const isModded = data && data._modded;
        const isEdited = data && data._edited;

        return `
            <div class="item-card ${isModded ? 'modded' : ''} ${isEdited ? 'edited' : ''}" data-name="${name}" data-category="${category || ''}">
                <div class="card-header">
                    ${color ? `<span class="card-color" style="background-color: ${color}"></span>` : ''}
                    <span class="card-title">${displayName}</span>
                    <div class="card-actions">
                        ${isModded ? '<span class="card-badge modded-badge" title="Modified by mod">MOD</span>' : ''}
                        ${isEdited ? '<span class="card-badge edited-badge" title="Edited">EDITED</span>' : ''}
                        <button class="card-edit-btn" title="Edit ${displayName}" data-name="${name}" data-category="${category || ''}">
